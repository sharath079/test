...
function getPageDataInfo()
{
	...
	dataObj.objectType = "ConfigProperties";
    $.ajax({
        context: {
            'errorMessage': "abcd"
        },
        ...
        success: function (responseData) {
            ...
            if (responseData['success'] == 1)
            {
                var fieldsDataWithoutOverrideWhereClause = responseData['fieldsDataWithoutOverrideWhereClause'];                
                	...
                	var configProperties = fieldsDataWithoutOverrideWhereClause['configProperties'];                
		                
		                
		                var propertyName = configProperties['propertyName'];
		                $('[data-name="PropertyName"]').text(propertyName);
		                
		                
		                var propertyValue = configProperties['propertyValue'];
		                $('[data-name="PropertyValue"]').text(propertyValue);
