...
function getPageDataInfo()
{
	...
	dataObj.objectType = "TaskScheduleInfo";
    $.ajax({
        context: {
            'errorMessage': "abcd"
        },
        ...
        success: function (responseData) {
            ...
            if (responseData['success'] == 1)
            {
                var fieldsDataWithoutOverrideWhereClause = responseData['fieldsDataWithoutOverrideWhereClause'];                
                	...
                	var taskScheduleInfo = fieldsDataWithoutOverrideWhereClause['taskScheduleInfo'];                
		                
		                
		                var taskInfo = taskScheduleInfo['taskInfo'];
		                $('[data-name="TaskInfo"]').text(taskInfo);
		                
		                
		                var targetEntityId = taskScheduleInfo['targetEntityId'];
		                $('[data-name="TargetEntityId"]').text(targetEntityId);
		                
		                
		                var targetUserId = taskScheduleInfo['targetUserId'];
		                $('[data-name="TargetUserId"]').text(targetUserId);
		                
		                
		                var notificationMedium = taskScheduleInfo['notificationMedium'];
		                $('[data-name="NotificationMedium"]').text(notificationMedium);
		                
		                
		                var notificationLastSentTime = taskScheduleInfo['notificationLastSentTime'];
		                $('[data-name="NotificationLastSentTime"]').text(notificationLastSentTime);
		                
		                
		                var nextNotificationTime = taskScheduleInfo['nextNotificationTime'];
		                $('[data-name="NextNotificationTime"]').text(nextNotificationTime);
